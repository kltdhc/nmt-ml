...
class BasicDecoder(decoder.Decoder):
    ...
    def __init__(self, cells, helpers, initial_states, output_layers=None, prop=None, dense_layer=None):
        ...
        for i in range(len(cells)):
            ...
            if not isinstance(helpers[i], helper_py.Helper):
                raise TypeError("helper[%d] must be a Helper, received: %s" % (i, type(helper[i])))
        self._cells = cells
        ...
        if prop is None:
            ...
            self.prop = tf.Variable(prop, name='ml_prop')
        else:
            self.prop = prop
        self.prop = tf.reshape(self.prop, [1,-1,1])
        # if dense_layer is None:
        #     self.dense_layer = tf.layers.Dense(
        #         1, 
        #         dtype=self.prop.dtype.base_dtype)
        # else:
    ...
    def initialize(self, name=None):
        ...
        first_inputs = []
        for i in range(len(self._initial_states)):
            rt = self._helpers[i].initialize()
            finished = rt[0]
            first_inputs.append(rt[1])
        return (finished, first_inputs, self._initial_states)
    ...
    def step(self, time, inputs, states, name=None):
        ...
        with tf.variable_scope("MultiDecoderScope"):
            ...
            # prop = tf.minimum(tf.maximum(self.prop, 0.5), 1.5)
            # outputs = tf.concat([tf.expand_dims(o, axis=-1) for o in outputs], -1)
            # outputs = tf.reduce_max(tf.nn.conv1d(outputs, prop, 1, 'SAME'), axis=-1)

            # outputs = tf.concat(outputs, -1)
            # with tf.variable_scope('ml_prop'):
                # outputs = self.dense_layer.apply(outputs)
                # outputs = tf.reduce_mean(outputs, axis=-1)
            outputs = tf.add_n(outputs)
            # outputs = outputs[0]

            sample_ids = self._helpers[0].sample(time=time, outputs=outputs, state=cell_state[0])
            
            for i in range(len(self._cells)):
                (finished, next_inputs, next_state) = self._helpers[i].next_inputs(
                    time=time,
                    outputs=outputs,
                    state=cstates[i],
